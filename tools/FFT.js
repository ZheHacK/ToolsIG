...
},
{
  ...
  message:'[>] Jumlah Akun per Penundaan:',
  validate: function(value){
...
},
{
  type:'input',
  name:'sleep',
  message:'[>] Insert Sleep/Delay (Milidetik):',
  validate: function(value){
    value = value.match(/[0-9]/);
    if (value) return true;
...
const Excute = async function(User, TargetUsername, Sleep, accountsPerDelay){
  try {
    ...
    console.log(chalk`{yellow  [!] Login Succsess, }{green [?] Try To Get ID & Followers Target . . .}`)
...
console.log(chalk`
  ...
  [?] {bold.green FFTComment | Using Account/User Target!}

    ——————————————————  [CODE BY]  ————————————————————
  [✓] CODE BY (ZheAlHaqy)
  [✓] FIXING & TESTING BY SYNTAX (Muslim Cyber Army)
  [✓] Thanks To Original (Instagram-private-api)
  —————————————————————————————————————————————————————}
      `);
